...
def _index(rows: Any, key: str, label: str) -> dict[str, dict[str, Any]]:
    ...
    for row in rows:
        ...
        ident = row.get(key)
        _need(
            isinstance(ident, str) and bool(ident) and ident not in result,
            f"bad/duplicate {label} ID",
        )
        result[ident] = row
    return result


def _identity_value(obj: dict[str, Any], key: str, label: str) -> str:
    _need(key in obj, f"{label} missing required identity {key}")
    value = obj[key]
    _need(isinstance(value, str) and bool(value), f"{label} has invalid identity {key}")
    return value
...
def _validate_manifest(manifest: dict[str, Any]) -> tuple[set[str], set[str], set[str]]:
    ...
    for pid, passage in pmap.items():
        ...
        _need(
            isinstance(sid, str) and sid in smap,
            f"passage {pid} references unknown source",
        )
        locator = passage.get("locator")
        representation = passage.get("representation_identity")
        _need(
            (isinstance(locator, str) and bool(locator))
            or (isinstance(representation, str) and bool(representation)),
            f"passage {pid} lacks locator/representation",
        )
...
def _validate_judgments(query: dict[str, Any], passage_ids: set[str]) -> None:
    _need(
        "judgments" in query and "groups" in query,
        "gold query must contain judgments and groups",
    )
    judgments = query["judgments"]
    groups = query["groups"]
    _need(
        isinstance(judgments, list) and isinstance(groups, list),
        "judgments/groups must be lists",
    )
    ...
    for j in judgments:
        ...
        gain = j.get("gain")
        _need(
            isinstance(pid, str) and pid in passage_ids and pid not in seen_j,
            "bad/duplicate judgment passage",
        )
        _need(degree in VALID_DEGREES and role in VALID_ROLES, "invalid degree/role")
        if degree == "UNKNOWN":
            _need(
                binary is None and gain is None and role == "UNKNOWN",
                "UNKNOWN must have null binary/gain and role UNKNOWN",
            )
        else:
            _need(
                type(binary) is bool and type(gain) is int and gain >= 0,
                "bad resolved judgment",
            )
            _need(
                (binary and gain > 0) or ((not binary) and gain == 0),
                "binary/gain inconsistency",
            )
        seen_j.add(pid)
    ...
    for group in groups:
        ...
        _need(
            isinstance(gid, str) and bool(gid) and gid not in seen_g,
            "bad/duplicate group ID",
        )
        ...
        _need(
            len(set(members)) == len(members) and set(members) <= passage_ids,
            "duplicate/unknown group member",
        )
        seen_g.add(gid)
...
def _ndcg_ok(query: dict[str, Any], gold: dict[str, Any]) -> bool:
    if (
        gold.get("ndcg_eligible") is not True
        or gold.get("qrels_mode") != "complete_relevant_set"
    ):
        return False
    rows = query["judgments"]
    if any(j.get("binary_relevant") is None for j in rows):
        return False
    positive_levels = {
        j.get("gain")
        for j in rows
        if type(j.get("gain")) is int and j.get("gain") > 0
    }
    return len(positive_levels) >= 2


def _score(
    query: dict[str, Any], ranked: list[str], k: int, allow_ndcg: bool
) -> dict[str, float | None]:
    top = ranked[:k]
    got = set(top)
    judgments = {j["passage_id"]: j for j in query["judgments"]}
    relevant = {pid for pid, j in judgments.items() if j["binary_relevant"] is True}
    support = {
        pid
        for pid, j in judgments.items()
        if j["binary_relevant"] is True and j["role"] == "SUPPORT"
    }
    counter = {
        pid
        for pid, j in judgments.items()
        if j["binary_relevant"] is True and j["role"] == "COUNTEREVIDENCE"
    }
    ...
    if allow_ndcg:
        ...
        ideal = sorted(
            (j["gain"] for j in judgments.values() if j["gain"] is not None),
            reverse=True,
        )[:k]
        dcg = sum(
            ((2**gain) - 1) / math.log2(i + 2) for i, gain in enumerate(observed)
        )
        idcg = sum(
            ((2**gain) - 1) / math.log2(i + 2) for i, gain in enumerate(ideal)
        )
    ...
    return {
        ...
        "judgment_coverage_at_k": (
            1.0 if not top else sum(pid in judgments for pid in top) / len(top)
        ),
        "resolved_judgment_coverage_at_k": (
            1.0
            if not top
            else sum(
                pid in judgments and judgments[pid]["binary_relevant"] is not None
                for pid in top
            )
            / len(top)
        ),
...
def evaluate(
    manifest: dict[str, Any], gold: dict[str, Any], run: dict[str, Any]
) -> dict[str, Any]:
    ...
    for key in ("corpus_version", "corpus_sha256", "benchmark_sha256"):
        mv = _identity_value(manifest, key, "manifest")
        gv = _identity_value(gold, key, "gold")
        rv = _identity_value(run, key, "run")
        _need(mv == gv == rv, f"identity mismatch: {key}")
    ...
    for qid, row in rrows.items():
        ...
        for hit in hits:
            ...
            _need(
                isinstance(pid, str) and pid in passages and type(rank) is int,
                "bad hit identity/rank",
            )
            ids.append(pid)
            ranks.append(rank)
        _need(
            ranks == list(range(1, len(ids) + 1)) and len(ids) == len(set(ids)),
            "rank/duplicate violation",
        )
        ranked[qid] = ids

    ndcg_flags = {qid: _ndcg_ok(gmap[qid], gold) for qid in sorted(gmap)}
    per = {
        qid: _score(gmap[qid], ranked[qid], k, ndcg_flags[qid])
    ...
    return {
        ...
        "metric_interpretation": (
            "lower_bound" if mode == "partial" else "point_estimate"
        ),
