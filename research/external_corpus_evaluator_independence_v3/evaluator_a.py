...
def _required_identity(obj: dict[str, Any], key: str, label: str) -> str:
    if key not in obj:
        raise ContractError(f"{label} missing required identity: {key}")
    value = obj[key]
    if not isinstance(value, str) or not value:
        raise ContractError(f"{label} identity must be a non-empty string: {key}")
    return value
...
def _validate_envelope(
    ...
    for key in ("corpus_version", "corpus_sha256", "benchmark_sha256"):
        mv = _required_identity(manifest, key, "manifest")
        gv = _required_identity(gold, key, "gold")
        rv = _required_identity(run, key, "run")
        if gv != mv or rv != mv:
    ...
    for pid, passage in passages.items():
        ...
        if not (
            ...
            raise ContractError(
                f"passage {pid!r} lacks reconstructable locator/representation identity"
            )
...
def _validate_judgment(j: dict[str, Any], corpus_ids: frozenset[str]) -> None:
    ...
    if degree == "UNKNOWN":
        if binary is not None or gain is not None or role != "UNKNOWN":
            raise ContractError("UNKNOWN requires null binary/gain and role UNKNOWN")
        return
    if type(binary) is not bool or type(gain) is not int or gain < 0:
        raise ContractError(
            "resolved judgments require boolean relevance and non-negative integer gain"
        )
...
def _index_gold(
    gold: dict[str, Any], ctx: EvalContext
    ...
    for q in rows.values():
        if "judgments" not in q or "groups" not in q:
            raise ContractError("gold queries must explicitly contain judgments and groups")
        ...
        for g in groups:
            ...
            if (
                ...
                or not members
                or not all(isinstance(x, str) and x for x in members)
            ):
    ...
    for qid, row in rows.items():
        ...
        for rank, hit in enumerate(hits, 1):
            if (
                ...
                or type(hit.get("rank")) is not int
...
def _query_ndcg_eligible(q: dict[str, Any], gold: dict[str, Any]) -> bool:
    ...
    gains = {
        j["gain"]
        for j in judgments
        if type(j.get("gain")) is int and j["gain"] > 0
    }
    return len(gains) >= 2


def _query_metrics(
    q: dict[str, Any], hits: list[str], k: int, allow_ndcg: bool
) -> dict[str, float | None]:
    top = hits[:k]
    got = set(top)
    judgments = {j["passage_id"]: j for j in q["judgments"]}
    relevant = {p for p, j in judgments.items() if j["binary_relevant"] is True}
    support = {
        p
        for p, j in judgments.items()
    ...
    if groups:
        ...
        for g in groups:
            ...
            satisfied += (
                members <= got
                if g["group_kind"] == "JOINTLY_REQUIRED"
    ...
    if allow_ndcg:
        dcg = sum(
            ((2 ** (judgments.get(pid, {}).get("gain") or 0)) - 1)
            / math.log2(rank + 1)
...
def evaluate(
    ...
    ndcg_by_query = {
        qid: _query_ndcg_eligible(gold_by_q[qid], gold)
        for qid in sorted(gold_by_q)
    }
    per_query = {
        qid: _query_metrics(
            gold_by_q[qid], run_by_q[qid], ctx.k, ndcg_by_query[qid]
        )
        for qid in sorted(gold_by_q)
    }
    metric_names = (
    ...
    aggregate = {
        name: _macro([metrics[name] for metrics in per_query.values()])
